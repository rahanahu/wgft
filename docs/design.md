...
- TCP の packet_rate の旨のレビュー指摘を直す(2026-09-20、7a.9 節):レビューで 3 つの食い違いが見つかった。Web UI(`ShowPacketNote`)が TCP のルールなら値の有無に関わらず旨を出しており、上の改訂の記録(2026-09-20「TCP の packet_rate を仕上げる」)にはそう書いたが、本文(7a.9 節「TCP の packet_rate」)と Phase 5 の変わるものの一覧はもともと「`packet_rate` が保存されているとき」の条件を意図していたので、実装をこの条件に合わせた。`rule add` は `packet_rate` を設定するフラグを持たず、旨を出す経路に乗らないので、本文と手順 5 の一覧から `rule add` を外した(実装側の同名の呼び出しも削除)。CLI の該当試験は、旨を stderr に出す実装に対して stdout だけを見ており、条件を壊しても落ちなかったので、stderr を見るように直し、条件を壊すと落ちることを確かめた。この項目自体は Admission Policy の判定を変えない
