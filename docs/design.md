...
- 値の誤り:`WGFT_WG_PORT` と `WGFT_MTU` が整数でない、`WGFT_MAX_UDP_FLOWS` と `WGFT_MAX_TCP_FLOWS` が範囲外、`WGFT_MAX_UDP_FLOWS_PER_SOURCE` と `WGFT_MAX_TCP_FLOWS_PER_SOURCE`(`vpsd` だけ)が負または範囲外
