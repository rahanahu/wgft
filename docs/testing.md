...
ラボの一式の拡張のうち、レートの判定 (L3 の拡張) と Resource Guard の場面 (L8 の拡張) も v1 に必須です。どちらも実装した時点でラボの一式に加わり、以後はコードを変える PR ごとに流れます。v1 の条件のうち、版の組み合わせ (B7)、公式に対応をうたう 3 つのディストリビューションでの配布物の確認 (B9) は、v1.0 を出す前に自動の試験として揃えます。
