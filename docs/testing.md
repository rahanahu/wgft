...
次の表の契機に当たるパスを PR が変えたときは、B 類の列のテストのうち実装済みのものを流します。新設の B 類 (B8、B9) は、実装されるまで契機に当たっても流しません。1 つのパスが複数の契機に当たる場合は、当たる契機のテストをすべて流します。ラボの一式の列は、開発の途中で流し直す確認を選ぶための目安で、マージの前にはラボの一式のすべてを流します。
