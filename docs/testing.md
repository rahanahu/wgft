...
- 足りない理由:実装したのはラボの netns トポロジ 1 台の上でのプロセス単位の入れ替えで、B9 が使う想定の systemd の unit と実際の VM の再起動は経由しません。B9 自体がまだ実装されていないため (前述の「B9 配布物の VM 試験」)、その環境での更新はまだ確かめられません。旧版へ戻したときの挙動も、判定をせずに記録するだけで自動化していません (前述の「更新と戻しの約束」)。userspace モードでは server 自身の WireGuard 公開鍵を CLI からも API からも読めないため、鍵が変わらないことは agent 側の公開鍵でだけ確かめています。1 本のルールがフロー予算をすべて使えることの再検証は L8 の役目なので、D4 では Resource Guard の行が出ることの確認にとどめます
- 環境:ラボです
- 時期:リリース候補ごとに流します
- 契機:`rc`
- 自動化:自動です。開発者が起動します
- v1:必須で、リリース候補ごとの項目です。B9 の環境 (実際の systemd unit、VM の再起動) での更新の確認は、B9 を実装した時点で別途加えます
