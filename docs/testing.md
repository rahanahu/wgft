...
- C 類 (段階の完了の関門):[設計文書](design.md) 7a.8 節の段階 (Phase 5、6、7 など) を終えるときに流すテストです。悪い条件のネットワーク、クラッシュからの回復、別のディストリビューションでのラボの一式を含みます
- D 類 (リリース候補の関門):リリースの候補の版で流すテストです。Windows と macOS のエージェント、旧版からの更新、長時間の通信、小さいメモリの環境を確かめます
...
コードを変える PR は、マージの前にラボの一式 (A9) を両モードで流します。文書だけを変える PR は、ラボを流しません。CI の検査 (A1 から A8) は、どちらの PR でも CI が流します。
...
B 類の契機は、ラボの一式に含まれないテスト (B 類の一覧) を選ぶためと、開発の途中でラボの確認を流し直すときに、変えた領域の確認だけを選ぶために使います。開発の途中の選び方は、後述の「ラボの一式の内訳」の表の契機の列にあります。

## 変更の契機と対象のパス

次の表の契機に当たるパスを PR が変えたときは、B 類の列のテストを流します。1 つのパスが複数の契機に当たる場合は、当たる契機のテストをすべて流します。ラボの一式の列は、開発の途中で流し直す確認を選ぶための目安で、マージの前にはラボの一式のすべてを流します。
...
| `admission` | `internal/policy/**`、`internal/dataplane/userspace/srcpolicy/**`、`internal/flowcap/**` (Phase 6 までは送信元ごとの上限を含むため)、`proto/rate.go`、`proto/source.go` | B1 | L2、L3、L13 |
| `resource` | `internal/flowcap/**`、`internal/resource/**` (Phase 6 で置く)、`internal/dataplane/userspace/**`、`internal/netpipe/**`、`internal/nettun/**`、`cmd/wgft/limits.go` | 無し (D1 と D2 を流し直す) | L8 (両モード) |
| `reconcile` | `internal/reconcile/**`、`internal/planner/**`、`internal/model/**`、`internal/dataplane/*.go`、`internal/vpsd/apply.go`、`internal/vpsd/watch.go`、`internal/vpsd/dataplane*.go` | 無し (C2 を流し直す) | L4、L5、L6、L9、L10 (両モード) |
...
| `dataplane-net` | `internal/dataplane/**`、`internal/nettun/**`、`internal/netpipe/**`、`internal/agent/**` の転送の経路 | 無し (C1、D1 を流し直す) | L1 |
...
今の CI は、B4 (`windows-test`)、B5 (`release-snapshot`)、B6 (`govulncheck`) を契機を問わずすべての PR で流しています。契機に合わせて振り分ける仕組み (パスによる絞り込みと、B6 の定期実行) は未実装で、v1.1 以降に導入します。それまでは、この 3 つが関係の無い PR で落ちた場合も、その PR の不具合とは限りません。B6 は外部の脆弱性の情報によって結果が変わるので、コードを変えていない PR でも落ちることがあります。
...
| A9 | ラボの一式 (L1 から L13。モードを持つ確認は両モードで) | 領域をまたぐ変更の見落としを含む、結合したときの退行全般。7a.8 節の共通の完了条件 | ラボ (使い捨て VM で並列、または 1 台で順に) | コードを変える PR、`phase`、`rc` | マージの前に 1 回 | 並列で約 2.5 分、1 台で 10 分以上 | 自動 (開発者が起動。L3 は半自動) |
...
| L3 | `lab/rates.sh` (kernel と userspace) | 3 つのレートの実際の通過数が両モードで食い違うこと | ラボ | `admission`、`nft-emit`、`relay`、`userspace` | A9 として | モードごとに約 35 秒 | 半自動 (判定の追加は新設) |
...
| L13 | IPv6 の送信元の拒否 (新設) | IPv6 の送信元が deny をすり抜けること、IPv6 のフラッドが集約のレートのトークンを使うこと | ラボ (IPv6 を加えた netns) | `admission`、`relay` | A9 として | 1 分前後 (見込み) | 自動 (新設) |
...
| 番号 | テスト | リスク | 環境 | 契機 | 頻度 | 所要時間 | 自動化 |
|---|---|---|---|---|---|---|---|
| C1 | 悪い条件のネットワーク (新設) | 損失、遅延、小さい経路 MTU と ICMP の遮断、自宅側のアドレスの変化で、トンネルと転送が回復しないこと | ラボ (netns に `tc netem` と経路の変更を加える) | `phase`、`dataplane-net` | v1 の前に 1 回、以後は契機に当たるとき | 10 分前後 (見込み) | 自動 (新設) |
| C2 | クラッシュと強制停止からの収束 (新設) | Prepare と Commit の間での強制終了や VM の強制停止の後に、宣言した状態へ収束しないこと | ラボ | `phase`、`reconcile` | v1 の前に 1 回、以後は契機に当たるとき | 数分 (見込み) | 自動 (新設) |
| C3 | 規模の試験 (新設) | ルール数とエージェント数が多いときの適用時間、テーブルの差し替え、全体状態の大きさの問題 | ラボ | `phase` | 段階の完了ごとに 1 回 | 10 分前後 (見込み) | 自動 (新設) |
| C4 | ラボの一式を別のディストリビューションで | カーネルと nftables の版の違いによる挙動の違い (通知、`ct count`、式の表記) | ラボ (`WGFT_LAB_IMAGE` で別のイメージの VM) | `phase`、`kernel` | v1 の前に 1 回、以後は契機に当たるとき | A9 と同じ | 自動 (開発者が起動。Fedora は未対応) |

### D 類 (リリース候補の関門)

| 番号 | テスト | リスク | 環境 | 契機 | 頻度 | 所要時間 | 自動化 |
|---|---|---|---|---|---|---|---|
| D1 | 長時間の TCP と UDP (新設) | WireGuard の鍵の更新、`rotate-key`、conntrack の期限、ハートビートをまたいだ長いセッションの切断 | ラボ | `resource`、`dataplane-net` | v1 の前に 1 回、以後は契機に当たるとき | 1 時間以上 (見込み) | 自動 (新設) |
| D2 | 小さいメモリの環境 (新設) | 256 MiB の VPS の目安の設定で、フラッドの下で server が OOM で落ちること | ラボ (メモリを制限した cgroup) | `resource`、`dataplane-net` | v1 の前に 1 回、以後は契機に当たるとき | 数分 (見込み) | 自動 (新設) |
...
| D5 | リリースの成果物と署名の検証 | 成果物の欠け、`wgft version` の表記の誤り、`gh attestation verify` の失敗、GHCR のイメージの欠け | リリースの後の GitHub と GHCR | `rc` (タグの後) | リリースごとに 1 回 | 10 分前後 | 半自動 (成果物の名前は B5 が確かめる) |
| D6 | 旧版からの更新 (新設) | 直前のリリースから更新したときにサーバのデータベースと認証情報が読めないこと、更新の間に kernel モードの転送が途切れること | B9 と同じ 2 台の VM | `rc` | リリース候補ごとに 1 回 | 10 分前後 (見込み) | 自動 (新設) |
...
|---|---|---|---|---|---|---|---|
| F1 | conntrack のエントリ 1 件のカーネルのメモリ (slab) | `nf_conntrack_max` の提示値に添えるメモリの値 (7a.10 節) | ラボ | Phase 6、カーネルの対応範囲の変更 | v1 の前に 1 回 | 1 時間前後 | 提示の文言の単体テスト (固定の値を使う) |
...
リリース候補ごとに流す項目は次のとおりです。ラボの一式に含まれる項目 (L1、L13) は、コードを変える PR ごとにも流れます。
...
- 対応する OS での smoke:D3 (Windows)、D4 (macOS)、B8 (macOS の runner)

v1 の前に 1 回流し、以後は関係する領域を変えたときにだけ流し直す項目は次のとおりです。

- 256 MiB の環境でのフラッド:D2。Resource Guard か転送の経路を変えたときに流し直します
- 1 時間以上の通信:D1。Resource Guard か転送の経路を変えたときに流し直します
- conntrack のエントリ 1 件のメモリの測定:F1。対応するカーネルの範囲を変えたときに流し直します
...
ラボの一式の拡張のうち、レートの判定 (L3 の拡張) と Resource Guard の場面 (L8 の拡張) も v1 に必須です。どちらもラボの一式に入るので、コードを変える PR ごとに流れます。v1 の条件のうち、ラボの IPv6 の経路 (L13)、版の組み合わせ (B7)、公式に対応をうたう 3 つのディストリビューションでの配布物の確認 (B9) は、v1.0 を出す前に自動の試験として揃えます。

## 更新と戻しの約束

更新の経路は保証します。旧版への戻しは互換性の契約に含めず、各版で観測した挙動だけを記録します。戻す必要があるときは、更新の前に取ったデータの置き場のバックアップから戻します。戻しを約束すると、サーバのデータベースのスキーマ、migration、知らないフィールドの保存、状態ファイル、wire protocol の変更を、旧い版が読める形に永久に縛るためです ([設計文書](design.md) 7a.6 節)。D6 はこの約束に従い、更新を確かめ、戻しについては挙動を記録するだけにします。

## 新設と自動化が未了の項目

各項目は、確かめる内容、今のラボで足りない理由、必要な環境、流す時期、流す契機、自動か手作業か、v1 に必須か v1.1 以降でよいか、の 7 点で定めます。v1 の項目には、前節の繰り返しの頻度を付けます。

### L3 の拡張:レートの判定と Relay のレート
...
- 環境:ラボです
- 時期:ラボの一式 (A9) として、コードを変える PR ごとに流します
...
- 時期:ラボの一式 (A9) として、コードを変える PR ごとに流します
- 契機:開発の途中では `admission`、`relay`
- 自動化:自動です
- v1:必須で、リリース候補ごとの項目です。v1 は IPv4 だけを扱う (4 節) ので、この守りが崩れると IPv6 の送信元が deny の一覧を通り抜けます
...
- 足りない理由:ラボの一式は、同じ版の server と agent だけを組み合わせます。版の交渉の論理は単体テスト (`proto/version_test.go`、`internal/vpsd/stream`) が確かめますが、実際に配布した旧い版のバイナリとの組み合わせは確かめません。wire protocol の版と機能の交渉を導入したときは、7a.8 節の完了条件として、この組み合わせをラボで手作業で確かめました
- 環境:直前のリリースのバイナリを VM に置いたラボです
- 時期:契機に当たる PR ごとと、リリース候補ごとに流します
- 契機:`protocol`、`rc`
- 自動化:自動です
- v1:必須で、リリース候補ごとの項目です。server と agent は v1.0.x の間 legacy v0 を必ず支えると定めている (7a.6 節) ので、その約束を確かめる手段が要ります

### B8 macOS の runner での単体テスト
...
- 時期:契機に当たる PR の更新ごとと、リリース候補ごとに流します
- 契機:`agent-platform`、`rc`
- 自動化:自動です
- v1:必須で、リリース候補ごとの項目です。追加の費用は CI の 1 ジョブだけで、Mac の実機での確認 (D4) の前に退行を捉えられます。runner の macOS が実機と同じ送信バッファの既定を持つかは未確認です
...
- 時期:v1 の前に 1 回流し、以後は転送の経路を変えたときと段階の完了で流し直します
...
- 時期:v1 の前に 1 回流し、以後は収束の仕組みを変えたときと段階の完了で流し直します
- 契機:`reconcile`、`phase`
- 自動化:自動です
...
- 時期:v1 の前に 1 回流し、以後は kernel 側の経路を変えたときと、kernel 側を変える段階の完了で流し直します
- 契機:`kernel`、`phase`
- 自動化:自動です。開発者が起動します
...
- 足りない理由:ラボの一式のフローは、数秒から数十秒で終わります
- 環境:ラボです
- 時期:v1 の前に 1 回流し、以後は Resource Guard か転送の経路を変えたときに流し直します
- 契機:`resource`、`dataplane-net`
...
- 時期:v1 の前に 1 回流し、以後は Resource Guard か転送の経路を変えたときと、目安の値を変えたときに流し直します
- 契機:`resource`、`dataplane-net`
- 自動化:自動です
...
### F1 conntrack のエントリ 1 件のメモリの測定

- 内容:ラボで conntrack のエントリ 1 件が使うカーネルのメモリを測り、`server check` と起動時の提示に添える値を決めます (7a.10 節の移行の手順 5)
- 足りない理由:測定の値は今の文書とコードに無く、提示の文言の単体テストは固定の値しか扱えません
- 環境:ラボです
- 時期:v1 の前に 1 回測り、以後は対応するカーネルの範囲を変えたときに測り直します
- 契機:Phase 6、カーネルの対応範囲の変更
- 自動化:手作業の測定です。測った値は提示の文言の単体テスト (A2) で守ります
...
- 内容:B4、B5、B6 を契機のパスに当たる PR でだけ流し、B6 を週に 1 回の定期実行とリリースのタグでも流します
...
| 関係する PR (B 類) | Windows の runner での単体テスト (B4) | macOS の runner での単体テスト (B8、新設) |
...
macOS の挙動は Linux の上で再現できるとはみなしません。macOS の CI の runner は単体テスト (B8) には使えますが、launchd、スリープと復帰、ネットワークの変化の確認は、実機での手作業の関門 (D4 と E5) にします。FileVault を無効にした Mac でのログイン無しの起動と、ログアウトの後の動作は未確認です。

### 実機の確認を小さな回帰テストに置き換える候補

エージェントの tunnel と userspace の server 側を 1 つのプロセスの中で接続し、TCP と UDP を転送する Go のテストを作れば、Windows と macOS の runner (B4、B8) で D3 と D4 の一部 (転送、大きな UDP) を PR ごとに確かめられます。userspace の server 側のコードが Windows と macOS でビルドできるかは未確認です。この置き換えは v1.1 以降の候補とし、置き換えた後も、launchd、ACL、スリープ、ネットワークの変化の確認は実機に残します。
...
4. 回帰テストは、実験の測定値そのものではなく、測定値から決めた上限や判定を確かめます。フロー 1 本のメモリの測定 (F5) を例に取ると、回帰テストは測定のやり直しではなく、上限までのフラッドで RSS がソフト上限と余裕の和の内側にあることの確認 (L8) です
