...
コードを変える PR は、マージの前にラボの一式 (A9) を両モードで流します。ラボの一式は、ラボの一式の内訳 (L 番号) のうち実装済みの確認の集まりで、今は L1 から L12 です。L13 と、L3 と L8 の拡張は、実装された時点でラボの一式に加わります。文書だけを変える PR は、ラボを流しません。CI の検査 (A1 から A8) は、どちらの PR でも CI が流します。
...
| A9 | ラボの一式 (L 番号のうち実装済みの確認。今は L1 から L12。モードを持つ確認は両モードで) | 領域をまたぐ変更の見落としを含む、結合したときの退行全般。7a.8 節の共通の完了条件 | ラボ (使い捨て VM で並列、または 1 台で順に) | コードを変える PR、`phase`、`rc` | マージの前に 1 回 | 並列で約 2.5 分、1 台で 10 分以上 | 自動 (開発者が起動。L3 は半自動) |
...
| L13 | IPv6 の送信元の拒否 (新設) | IPv6 の送信元が deny をすり抜けること、IPv6 のフラッドが集約のレートのトークンを使うこと | ラボ (IPv6 を加えた netns) | `admission`、`relay` | 実装後に A9 として | 1 分前後 (見込み) | 自動 (新設) |
...
ラボの一式の拡張のうち、レートの判定 (L3 の拡張) と Resource Guard の場面 (L8 の拡張) も v1 に必須です。どちらも実装した時点でラボの一式に加わり、以後はコードを変える PR ごとに流れます。v1 の条件のうち、ラボの IPv6 の経路 (L13)、版の組み合わせ (B7)、公式に対応をうたう 3 つのディストリビューションでの配布物の確認 (B9) は、v1.0 を出す前に自動の試験として揃えます。
...
- 自動化:自動です
- v1:必須で、リリース候補ごとの項目です。追加の費用は CI の 1 ジョブだけで、Mac の実機での確認 (D2) の前に退行を捉えられます。runner の macOS が実機と同じ送信バッファの既定を持つかは未確認です
...
macOS の挙動は Linux の上で再現できるとはみなしません。macOS の CI の runner は単体テスト (B8) には使えますが、launchd、スリープと復帰、ネットワークの変化の確認は、実機での手作業の関門 (D2 と E5) にします。FileVault を無効にした Mac でのログイン無しの起動と、ログアウトの後の動作は未確認です。
