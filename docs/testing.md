...
- 内容:2 本と 3 本のルールで 1 本にフラッドを掛けたまま、他のルールが予約まで新しいフローを通せることと、既定より小さい予算でも隔離が成り立つことを確かめます (7a.10 節)
- 足りない理由:今の check 5 は 1 本のルールが予算をすべて埋め、それを超えるフラッドを重ねたときの RSS だけを確かめており (Phase 6 の移行の手順 4 でこの形に改めました)、ルール間の隔離を確かめません。予算は、1 つの client の名前空間から出せるフラッドで埋められる値 (UDP 4096、TCP 1024) に下げてあり、既定の予算での RSS は確かめていません
