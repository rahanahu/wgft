...
コードを変える PR は、マージの前にラボの一式 (A9) を両モードで流します。ラボの一式は、ラボの一式の内訳 (L 番号) のうち実装済みの確認の集まりで、今は L1 から L13 です。文書だけを変える PR は、ラボを流しません。CI の検査 (A1 から A8) は、どちらの PR でも CI が流します。
...
ラボのスクリプトは、どれも自分で server、エージェント、宛先を立てて片付けるので、互いに独立しています。この独立性があるので、ラボの一式を複数の VM に分けて並列に流せます。複数の使い捨て VM で並列に流す道具を使うと、8 台で約 2.5 分に縮みます。この道具はリポジトリに含めていません。道具を持たない開発者は、1 台の VM で順に流します。その場合は 10 分以上かかります。`lab/lab` は `WGFT_LAB_VM` で VM の名前を変えられるので、同じ仕組みで複数の VM を立てられます。`lab/lifecycle.sh` の check 5、5b、5c、5d、5e はどれもフラッドで CPU を使うので、他の VM と CPU を取り合わない状態で流します。
...
v1 の条件のうち、公式に対応をうたう 3 つのディストリビューションでの配布物の確認 (B9) は、v1.0 を出す前に自動の試験として揃えます。版の組み合わせ (B7) は `lab/version-skew.sh` として実装済みです。ただし、旧い側が表せない機能のルールを理由付きの `not_active` にすることの確認だけは、該当する capability がまだ無いため未了です (後述の「B7 の not_active の確認」)。
