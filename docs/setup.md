...
- Targets: only IPv4 targets are forwarded, and loopback targets such as `127.0.0.1` are refused. For a service on the agent host itself, give the host's LAN address as the target. Such a service sees the server's tunnel address, `10.200.0.1` by default, as the source; a service on another LAN host sees the agent host's LAN address. A UDP service on the agent host should listen on that LAN address. An ordinary socket bound to all addresses, such as `0.0.0.0`, answers from the agent's tunnel address, so the reply does not match the forwarded flow and never reaches the client. Whether a service that picks its reply address itself is affected has not been verified. Using the same port for the rule's listen port and the target port does not help. TCP services, services on other LAN hosts and userspace mode are not affected. `wgft server doctor` does not report such a rule as failed, because it does not test UDP targets. [design.md](design.md) section 7b.2 explains the cause.
...
These steps have been verified on a Debian 12 VM in the development lab, with the server in kernel mode: TCP forwarding to the agent host's own address, UDP forwarding to a service listening on that address, TCP forwarding to another host, reboots, forwarding while the agent is stopped, `agent doctor` running and stopped, the exit code 3 without the drop-in, the teardown and the switch back and forth. On a staging machine, the drop-in has been verified in an unprivileged Debian 13 LXC container on Proxmox VE, with nesting enabled and the container's AppArmor profile unconfined. There, forwarding came back after `systemctl restart wgft-agent` and after restarting the container itself, forwarding continued while the agent was stopped for about 40 seconds, and `wgft agent rotate-key` worked with the agent running and with it stopped. Not verified: other distributions such as Ubuntu and Fedora, distributions that enforce SELinux or AppArmor, Proxmox VE containers with nesting disabled or with an AppArmor profile that confines them, Incus containers, and Docker. This guide has no kernel-mode steps for Docker.

v1.1.x has no kernel mode for the agent. To move a kernel-mode agent back to v1.1.x, run the steps for going back to userspace mode above with the v1.2 binary, except the final `systemctl start`, install the v1.1.x binary, then start the agent. On a staging machine, v1.1.3 then forwarded TCP, UDP and PROXY protocol v2 normally in userspace mode. Without the teardown, v1.1.3 still starts, but `wgft0` and `table inet wgft_agent` stay in the kernel, and v1.1.3 rewrites `agent.json` without the kernel-mode records. What the leftover `wgft0`, which still holds the agent's key, and the table do to forwarding has not been verified. `wgft agent teardown` of v1.2 still removes `wgft0` and the table after that, but it no longer finds the conntrack entries to remove and does not print the command to restore `ip_forward`, because v1.1.x dropped the records it needs.
...
Once the data plane is applied and the admin and agent APIs are listening, the server logs one `server started` line with its version, mode, generation, and the number of rules and agents. If that line is missing, the startup did not finish; the lines before it show where it stopped. One failure does not end the process: when the stored rules cannot be applied at all, the server logs a `startup hold` line, keeps only the admin API listening, and retries the apply every 30 seconds. `wgft rule rm` and `wgft rule disable` still reach it during the hold, so a declaration that cannot be applied can be made smaller. The server does not advance its active generation until the apply succeeds, and the `server started` line follows it. What the hold stops is publication, not forwarding. On a first boot or after the VPS itself reboots, there is nothing in the kernel yet, so nothing is forwarded. Across a bare restart of the process, whatever the previous process left in the kernel stays. Which declaration it carries depends on where the apply failed and cannot be told from the log. A failure before the kernel swaps the table, whether the update fails to send or the kernel rejects the batch, leaves the table genuinely unchanged, so it keeps forwarding under the previous declaration; a failure after the kernel has already swapped the table but before the reply reaches the server means the table now carries the declaration that was reported as failed. Proxy-mode listeners belong to the process and are gone either way. Read the actual kernel content with `wgft server nft`. Each successful rule change is logged with its origin, such as `cli rule add` or `ui import`, the affected rule IDs, and the resulting generation. To list only rule changes:
