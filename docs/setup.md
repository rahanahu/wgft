...
- Targets: only IPv4 targets are forwarded, and loopback targets such as `127.0.0.1` are refused. For a service on the agent host itself, give the host's LAN address as the target. Such a service sees the server's tunnel address, `10.200.0.1` by default, as the source; a service on another LAN host sees the agent host's LAN address. A UDP service on the agent host must listen on that LAN address. If it listens on all addresses, such as `0.0.0.0`, it answers from the agent's tunnel address, the reply does not match the forwarded flow, and it never reaches the client. Using the same port for the rule's listen port and the target port does not help. TCP services, services on other LAN hosts and userspace mode are not affected. `wgft server doctor` does not report such a rule as failed, because it does not test UDP targets. [design.md](design.md) section 7b.2 explains the cause.
...
v1.1.x has no kernel mode for the agent. To move a kernel-mode agent back to v1.1.x, first go back to userspace mode with `wgft agent teardown` as described above, while v1.2 is still installed, then install the older binary. On a staging machine, v1.1.3 then forwarded TCP, UDP and PROXY protocol v2 normally in userspace mode. Without the teardown, v1.1.3 still starts, but `wgft0` and `table inet wgft_agent` stay in the kernel, and v1.1.3 rewrites `agent.json` without the kernel-mode records. `wgft agent teardown` of v1.2 still removes `wgft0` and the table after that.
