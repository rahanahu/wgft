...
The release binary is not code-signed. Downloaded through a browser and started by double-clicking in Explorer, it triggers Microsoft Defender SmartScreen's "Windows protected your PC" warning, which blocks the app until you select More info and then Run anyway; approving it removes the download's mark, so the warning does not reappear on later starts. Depending on Windows or organization policy, the warning may not be bypassable. Starting the agent from PowerShell as this guide does avoids the warning entirely.
