...
Flows are capped at three levels: per rule, per source address, and process-wide. The agent cannot distinguish the original source address, so it applies only the per-rule and process-wide caps. In userspace mode, `wgft server` applies all three in Go. In kernel mode it does not relay, so it applies only the per-source cap, with nftables `ct count`. At a cap, new flows are refused and existing ones keep working. In kernel mode the home agent still relays the flows and is subject to its own caps.
