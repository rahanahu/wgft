...
- Privileges: the agent needs `CAP_NET_ADMIN`, and the host needs the kernel's WireGuard module. Without the capability, the agent stops at startup with exit code 3 and names `CAP_NET_ADMIN` in its log. It stops before it uses the join string or records the mode in `agent.json`, so removing `WGFT_MODE=kernel` starts it in userspace mode again. A kernel without WireGuard support stops the agent at the same point.
