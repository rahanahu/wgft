...
In kernel mode, a rule set is applied to the kernel as one nftables batch, sized to fit the netlink socket buffers it needs ([design.md](design.md) section 6.1). In the lab, the shipped systemd unit applied rule sets of up to 2000 rules in kernel mode without hitting a limit, both as a single `rule import` batch and as one generation per `rule add`. No currently documented kernel-mode deployment is limited by `net.core.rmem_max` or `net.core.wmem_max`: the shipped systemd unit grants the capability wgft needs to size its own buffers past those sysctls, and the Docker deployment documented above runs userspace mode only, not kernel mode.
