...
The release binary is not code-signed. Downloaded through a browser and started by double-clicking in Explorer, it triggers Microsoft Defender SmartScreen's "Windows protected your PC" warning, which blocks the app until you select More info and then Run anyway. Depending on Windows or organization policy, the warning may not be bypassable. On the Windows 11 system used for verification, choosing Run anyway prevented this SmartScreen prompt from appearing again for that file. Starting it from PowerShell as described above did not show this SmartScreen prompt.
