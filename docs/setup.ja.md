...
配布するバイナリはコード署名をしていません。Web ブラウザで取得したファイルを Explorer からダブルクリックで起動すると、Microsoft Defender SmartScreen の「Windows によって PC が保護されました」という警告が表示され、詳細情報を選んで実行を選ぶまでプログラムは起動しません。一度承認するとダウンロードに付いた目印が外れるため、次回以降はこの警告が表示されません。Windows または組織のポリシーによっては、この警告を回避できない場合があります。本書のとおり PowerShell から起動する場合、この警告は発生しません。
