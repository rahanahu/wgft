...
| `wgft server` のメモリ | フローの数によらず数十 MiB | フローの数に応じて増え、同時フロー数の上限で止まる |
...
wgft は、同時に中継するフローの数に上限を設けています。上限は、ルールごと、接続元アドレスごと、プロセス全体の 3 段です。上限に達すると新しいフローだけを拒否し、既存のフローは切りません。プロセス全体の上限は `WGFT_MAX_UDP_FLOWS` と `WGFT_MAX_TCP_FLOWS` で設定します。既定値はそれぞれ 8192 と 2048 です。`wgft server` と `wgft agent` のどちらにも設定できます。wgft は、この 2 つの値からランタイムのメモリのソフト上限を計算し、起動時に表示します。開発用のラボでは、ユーザー空間モードの server に対して全部の上限を埋め、さらに大量の通信を送りました。既定値のままの最大は 208 MiB でした。`WGFT_MAX_UDP_FLOWS=2048` と `WGFT_MAX_TCP_FLOWS=1024` を設定した場合は、同じ負荷を 150 MiB のメモリ制限の中で耐えました。150 MiB は、メモリが 256 MiB の VPS で wgft が使える量の目安です。その大きさの実際の VPS では未確認です。
