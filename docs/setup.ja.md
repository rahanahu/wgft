...
カーネルモードでは、ルール集合をカーネルへ 1 つの nftables バッチとして適用します。wgft はこのバッチに合わせて netlink ソケットのバッファの大きさを調整します([design.md](design.md) の 6.1 節)。ラボでは、付属の systemd unit によるカーネルモードで、1 回の `rule import` によるバッチでも 1 本ずつの `rule add` による追加でも、2000 本までのルール集合を上限に達せず適用できました。現在文書化されているカーネルモードの配置のうち、net.core.rmem_max や net.core.wmem_max で制限されるものはありません。付属の systemd unit は、wgft がバッファをその sysctl の上限を超えて調整するために必要な権限を与えます。上で説明した Docker での配置はユーザー空間モードだけに対応し、カーネルモードには対応していません。
