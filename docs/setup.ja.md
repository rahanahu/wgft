...
配布するバイナリはコード署名をしていません。Web ブラウザで取得したファイルを Explorer からダブルクリックで起動すると、Microsoft Defender SmartScreen の「Windows によって PC が保護されました」という警告が表示され、詳細情報を選んで実行を選ぶまでプログラムは起動しません。Windows または組織のポリシーによっては、この警告を回避できない場合があります。確認に使用した Windows 11 環境では、一度実行を選んだ同じファイルでは、この警告は再表示されませんでした。また、本書のとおり PowerShell から起動した場合、この警告は表示されませんでした。
