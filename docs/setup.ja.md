...
配布するバイナリはコード署名をしていません。Windows 11 の実機で確認したところ、Explorer から `wgft.exe` をダブルクリックすると発行元不明の警告が表示され、この警告をキャンセルするとプログラムは起動しません。上述の Windows Defender Firewall のダイアログでキャンセルしてもトンネルと中継が動作し続けるのとは異なります。本書のとおり PowerShell から起動する場合、この警告は表示されません。
