...
プルリクエストのタイトルも、コミットの 1 行目と同じ形 (`<type>(<scope>): <要点>`) にします。複数のコミットを含むときは、主な変更の `type` を使います。main にはスカッシュでマージし、main のコミットの 1 行目はタイトルになります。ただし、コミットが 1 つだけのプルリクエストでは、GitHub はタイトルではなくそのコミットの 1 行目を使います。このため、タイトルとコミットの 1 行目の両方をこの形にします。

リリースは、版の記述 (README の Status の節) を直すコミットを `chore(release): vX.Y.Z` の形で main に入れ、そのコミットにタグ `vX.Y.Z` を打ちます。版の上げ方は、前回のタグ以降に `feat` があれば minor、`fix` だけなら patch です (1.0 より前は、互換を崩す変更も minor で上げます)。
