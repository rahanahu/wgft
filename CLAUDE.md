...
`go test ./...` はホストで実行する単体テストで、ネットワーク namespace や root 権限を必要としません。`lab/` 配下の結合テストは Incus の VM を必要とするため、CI では実行されません。開発者は変更のたびにラボで結合テストを流し、CI には単体テストと後述の静的検査だけを任せます。どのテストをどの変更と時点で流すかは [docs/testing.md](docs/testing.md) に定めてあります。
