...
func TestLinuxOnlyServerGroupRefuses(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"check starts nothing", []string{"server", "check"}, "cannot continue " + subject},
		{"nft starts nothing", []string{"server", "nft"}, "cannot continue " + subject},
		{"teardown starts nothing", []string{"server", "teardown", "--purge", "--yes"}, "cannot continue " + subject},
		{"doctor starts nothing", []string{"server", "doctor"}, "cannot continue " + subject},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			r := startup.Of(err)
			if r == nil {
				t.Fatalf("error = %q, want a startup refusal so that the unit stops restarting it", err.Error())
			}
