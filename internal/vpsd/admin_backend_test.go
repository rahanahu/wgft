...
// TestAgentRuleStatusesWrapsStoreError confirms AgentRuleStatuses (admin.AgentRuleStatusBackend's
// implementation, design.md 5.2、7a.11 節) names the failed read, like Rules/Generation/RuleDrops
// above, instead of returning the store's raw error. admin.go's withAgentRuleStatus turns this into
// a 500 for the whole GET /api/v1/rules response (fail closed, design.md 10.5 節), so the operator
// needs to be able to tell which read failed from the message alone.
func TestAgentRuleStatusesWrapsStoreError(t *testing.T) {
	st, err := store.Open(filepath.Join(t.TempDir(), "s.sqlite"))
	if err != nil {
		t.Fatal(err)
	}
	st.Close() // force the subsequent store call to fail

	d := &Daemon{st: st, hub: stream.New(nil)}
	if _, err := d.AgentRuleStatuses(); err == nil || !strings.Contains(err.Error(), "reading agents") {
		t.Errorf(`AgentRuleStatuses() = %v, want an error naming the operation ("reading agents")`, err)
	}
}

// TestAgentRuleStatusesEmptyBeforeAnyHeartbeat confirms a registered agent that has not sent a
// heartbeat yet (hub.Status returns a zero Status, Heartbeat == nil) contributes no entries, rather
// than a per-rule zero value: a rule is simply absent until its agent has reported it at least once
// (design.md 5.2、7a.11 節, matching ResourceStatus.Refusals' convention). The richer cases (a
// refused target, a listener error, a disconnected agent's stale report) are exercised at the
// stream.Hub level by internal/vpsd/stream's own heartbeat tests (e.g. TestStream) and, for the
// admin API's JSON shape and the optional-Backend gating, by
// internal/vpsd/admin/agent_rule_status_test.go.
func TestAgentRuleStatusesEmptyBeforeAnyHeartbeat(t *testing.T) {
	...
	d := &Daemon{st: st, hub: stream.New(nil)}
	got, err := d.AgentRuleStatuses()
	if err != nil {
		t.Fatalf("AgentRuleStatuses() = %v, want success", err)
	}
	if len(got) != 0 {
