...
type serverDataplane interface {
	...
	// InputPortSuggestions は、指定した proto/port が input で塞がれていれば足す行を返す。プロキシ
	// モードの公開ポート(仕様 6.2 節)と、vpsd 自身の待ち受けポート(WireGuard の UDP、agent API の
	// TCP。仕様 4 節)の両方に使う。
	InputPortSuggestions(port uint16, p proto.Proto) ([]string, error)
...
func (k *kernelDataplane) InputPortSuggestions(port uint16, p proto.Proto) ([]string, error) {
	return linux.InputPortSuggestions(port, p, nft.TableName)
}
