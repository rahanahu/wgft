...
var tr = map[string][2]string{
	...
	"rateHelp":            {"超えた分は捨て、一覧の「拒否数」に数えます。いずれもバースト 5 です。", "Traffic over the limit is dropped and counted in the list's Denied column. All limits use a burst of 5."},
	"ratePerSourceHead":   {"接続元ごとの新規フロー", "New flows per source"},
	"rateNewFlowHead":     {"ルール全体の新規フロー", "New flows for the rule"},
	"ratePacketHead":      {"パケット", "Packets"},
