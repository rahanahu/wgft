...
var tr = map[string][2]string{
	...
	"unitDay":           {"日", "day"},
	"unitWeek":          {"週", "week"},
	// rateSummary*1 are the singular English forms, chosen in forms.gohtml with a direct
	// string comparison against .Count, e.g. eq .Rates.PerSource.Count "1". Count is a
	// string all the way from proto.Rate through the raw form value on a validation-error
	// re-display (webui_rule.go's field closure), so it may not even parse as an int in that
	// path; comparing against the literal "1" handles both cases without a parse step. This
	// mirrors groupErrN1/groupErrN and warnLinkFmt1/warnLinkFmt: a plain function like
	// pluralS(n int) doesn't apply here since these strings aren't always valid integers.
	"rateSummaryPerSourceFmt":  {"1 つの接続元から 1 %[1]sに %[2]s 本まで", "Up to %[2]s new connections per %[1]s from one source"},
	"rateSummaryPerSourceFmt1": {"1 つの接続元から 1 %[1]sに %[2]s 本まで", "Up to %[2]s new connection per %[1]s from one source"},
	"rateSummaryWholeFmt":      {"ルール全体で 1 %[1]sに %[2]s 本まで", "Up to %[2]s new connections per %[1]s for the whole rule"},
	"rateSummaryWholeFmt1":     {"ルール全体で 1 %[1]sに %[2]s 本まで", "Up to %[2]s new connection per %[1]s for the whole rule"},
	"rateSummaryPacketFmt":     {"1 %[1]sに %[2]s 個まで", "Up to %[2]s packets per %[1]s"},
	"rateSummaryPacketFmt1":    {"1 %[1]sに %[2]s 個まで", "Up to %[2]s packet per %[1]s"},
