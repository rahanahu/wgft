...
func TestAgentServerInProcessForwarding(t *testing.T) {
	...
	// 4. 握手を待ってから (壁時計で測る区間に入る前に状態の到達を確かめる。docs/testing.md
	// 「壁時計で測る区間を使うテストの規範」)、TCP と UDP を確かめる。予算は handshakeBudget を参照
...
// waitForHandshake は、after より後の握手が成立するまで待つ (壁時計で測る区間に入る前に状態の到達を
// 確かめる。docs/testing.md 「壁時計で測る区間を使うテストの規範」)。上限を超えたら失敗させ、診断のため
...
// (docs/testing.md「壁時計で測る区間を使うテストの規範」)ので、timeout は主張の根拠ではなく上限。
...
// 収束を待ってから比べる(壁時計で測る区間を使うテストの規範と同じ考え方)。
