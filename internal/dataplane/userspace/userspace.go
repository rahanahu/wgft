...
func New(opts Options) *Backend {
	...
	b.relay = relay.New(hostNetwork{}, relay.Options{
		...
		// 予算は relay が Limits から作る。Backend が別に作って渡すと、Pool を読む入口が 2 つになる
		Limits: opts.Limits,
	})
	return b
}
...
// (design.md 7, 7a.10 節). It is the relay's own pool: agent and server read the budget through the
// same accessor (design.md 10.2c 節).
func (b *Backend) TCPPool() *resource.Pool { return b.relay.TCPPool() }
...
func (b *Backend) UDPPool() *resource.Pool { return b.relay.UDPPool() }
