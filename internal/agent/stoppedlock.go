...
//     を消した後、一度も起動していないデータディレクトリがこの場合に当たる。最後の場合に書くのは
//     agent pubkey だけである(rotate-key は agent.json が無ければ書かない)。判定の直後に起動した
//     エージェントの書き込みを上書きし、agent pubkey が印字する鍵がエージェントの使う鍵と食い違いうる
//     狭い隙間は許容する(仕様 9 節)。
//
