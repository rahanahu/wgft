...
ダッシュボードは、エージェントの接続状態、ルールの適用状態と拒否数、警告、適用中の nftables の設定を 1 画面に表示します。表示は日本語と英語を切り替えられます。

この画面から行える操作は、接続文字列の発行、ルールの追加、有効化と無効化、削除、グループとメモの編集、TCP ルールの接続テスト、エージェントの無効化、警告の解除です。接続元の拒否と許可の一覧、レート制限、ルールの分割と統合、JSON からの一括置き換えは、[コマンド](#コマンド一覧)でだけ設定できます。Web UI はそれらの設定値と drop の累計を表示します。

ログイン画面はありません。管理 API は VPS の TCP ポートでは待ち受けず、Unix ソケット `/run/wgft/admin.sock` だけで待ち受けます。このソケットを開けるのは、root と `wgft server` 自身だけです。手元のブラウザからは、SSH に root でログインし、手元のポートをそのソケットへ転送して開きます。`ssh -L` の転送先にはポートだけでなくソケットのパスも書けます。
...
> ここから先の操作は、コマンドの代わりに [Web UI](#web-ui) でも行えます。この手順書は、コマンドで説明を続けます。Web UI で行う場合の操作は、該当する箇所に「Web UI では」として示します。

最後に、自宅のエージェント用の接続文字列を発行します。Web UI では、ダッシュボードの「+ エージェントを追加」で名前を入力して生成します。
...
VPS で `agent ls` を実行し、エージェントが登録されたことを確認します。Web UI では、ダッシュボードのエージェントの一覧に同じ内容が表示されます。
...
ルールを追加します。Web UI では、ダッシュボードの「+ ルールを追加」です。
