...
VPS で動かす `wgft server` には 2 つの動作モードがあります。`WGFT_MODE` で選び、初回の起動時に記録されます。自宅側の `wgft agent` は、どちらのモードでも同じものを同じ手順で使います。
...
VPS 側も自宅側も Linux で動きます。カーネルモードの server には、加えてカーネル 6.1 以上、nftables 1.0.6 以上、root 権限が必要です。Debian 12 と Ubuntu 24.04 以降がこれを満たします。ユーザー空間モードの server に追加の要件はなく、Docker が動けば十分です。以下はカーネルモードの補足です。WireGuard のパッケージを入れる必要はありません。カーネルモジュールはこれらのカーネルに含まれていて、server は `wg` や `wg-quick` を使わずにカーネルを直接操作します。カーネルにモジュールが無い VPS では、server はその旨を表示して止まります。その場合はユーザー空間モードで動かします。agent 側に追加の要件はなく、root 権限も TUN デバイスも必要ありません。
