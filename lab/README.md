...
lab/lab exec vm bash /wgft/lab/scale.sh kernel         # 規模の試験(C3)。1 台の server と 5 台のエージェントで、ルール数を 10 から 1000 まで段階的に増やし、適用時間・全体状態の大きさ・RSS を測定。kernel モードはこのラボ VM で概ね 100 本を超えると netlink のバッファの上限に当たり、FAIL で終わる(既知の問題。docs/testing.md の C3 と design.md の改訂の記録を参照)。userspace も同じ引数で流せ、こちらは 1000 本まで通る
